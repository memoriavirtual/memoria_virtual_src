...
public class UnmappedResourceHandler extends ResourceHandlerWrapper {
	...
	public boolean isResourceRequest(FacesContext context) {
		return ResourceHandler.RESOURCE_IDENTIFIER.equals(context
	...
	public void handleResourceRequest(FacesContext context) throws IOException {
		...
		if (libraryName == null || libraryName.equals("")) {
			for (BibliotecasParaBuscar b : BibliotecasParaBuscar.values()) {
				libraryName = b.toString();
		...
		for (Entry<String, String> header : resource.getResponseHeaders()
				.entrySet()) {
			externalContext.setResponseHeader(header.getKey(),
					header.getValue());
		}
		...
		try {
			...
			output = Channels.newChannel(externalContext
					.getResponseOutputStream());

			for (ByteBuffer buffer = ByteBuffer.allocateDirect(10240); input
