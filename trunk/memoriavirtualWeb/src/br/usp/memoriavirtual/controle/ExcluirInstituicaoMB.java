...
public class ExcluirInstituicaoMB implements Serializable {
	...
	/**
	 * Método é invocado a cada evento Ajax KeyUp no campo Nome da Instituição do formulário,
	 * tal campo está associado ao atributo this.nome, este atributo é utilizado para fazer uma 
	 * busca no banco de dados, que retorna uma lista em ordem alfabética, em relação ao nome, 
	 * de todas as instituições cujo o nome corresponde em parte com o atributo this.nome.   
	 * @param event
	 * Ajax KeyUp
	 */
	 ...
	 * as informações nos campos do formulário, o método this.listarGerentes() é invocado, e depois é retornado a 
	 * String Instselecionada que será interpretada pelo facesConfig	
	 * @return
	 * Se todas as validações retornarem true retorna a Sting Instselecionada 
	...
	/**
	 * Método invocado quando uma instituição é selecionada, seja na página do CDU ou no servlet,
	 * o método faz uma requisição no banco de dados de objetos Acesso relacionados com a instituição 
	 * selecionada, caso exista uma lista de objetos Acesso, ele transforma a lista em 
	 * uma lista de objetos Usuario e a atribui a this.gerentesInstituicao 
	 * e seta o atributo this.gerente com o  primeiro usuário da lista.
	 * caso não exista uma lista de objetos Acesso relacionados a Instituição,
	 * um objeto Usuario é criado mas este serve apenas para mostrar a mensagem na página 
	 * informando que a instituição não tem gerentes.  
	 * 
	 */
	...
	/**
	 * Método invocado quando ocorre o evento Ajax KeyUp na caixa de texto, onde o usuário preenche a
	 * justificativa.Este método apenas invoca o método this.contarJustificativa 
	 * @param event
	 */
	...
	/**
	 * Método faz analisa o tamanho da String exibe mensagens com o número de caracteres 
	 * na página do CDU, caso o número de caracteres exceda 300, a mensagem de erro também
	 * é exibida.   
	 * @return
	 * True caso o número de caracteres da justificativa seja inferior a 300 caracteres, do contrário
	 *  retorna falso. 
	 */
	 ...
	 * @return
	 * String confirmarexcluir, caso as validações dos campos sejam verdadeiras 
	 */
	 ...
	 * administradores e/ou gerentes que estão apitos a validarem o 
	 * pedido de exclusão. o método também trabalha para que a lista esteja lexamente ordenada, 
	 * para que o usuario da seção não esteja na lista e para que não contenha nomes repetidos
	 * @return
	...
	/**
	 * Método monta o texto do email, bem como chama o método responsável
	 * por enviar o mesmo.
	 * Ainda registra um objeto itemAuditoria, registra um objeto aprovação e 
	 * marca a instituição a ser excluída e os objetos Acesso relacionados a ela com o campo validade = false
	 * também trabalha com o objeto Date afim de encontrar a data de expiração que fara parte do objeto Aprovação
	 * @throws IOException
	 */
	...
	/**
	 * Método trata ação do botão Negar Pedido da Página do caso de uso 
	 * Nega o pedido, reativa a Instituição e todas as suas dependências 
	 * @return
	 */
	...
	/**
	 * Método  exclui a instituição do sistema Memória Virtual,bem como todos o objetos Acesso relacionados a ela
	 * , além disso, salva no bando de dados, na tabela auditoria
	 * os responsáveis pela exclusão
	 * 
	 */
	...
	/**
	 * Método reinicializa todos os atributos da classe ExcluirInstiuicaoMB.
	 * É utilizado nos métodos relacionados com os botões voltar das páginas do CDU 
	 * Excluir Instituição
	 */
